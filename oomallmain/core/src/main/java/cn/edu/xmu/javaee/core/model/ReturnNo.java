...
public enum ReturnNo {
     ...
     *  物流模块
     ************************************/
    FREIGHT_WAREHOUSEREGION_EXIST(997, "仓库已存在该配送地区(id=%d)"),//1
    FREIGHT_LOGISTIC_EXIST(998, "商铺已存在物流(id=%d)"),//2
    FREIGHT_LOGISTIC_NOEXIST(802,"不支持该物流"),
    FREIGHT_WAREHOUSEORLOGISTIC_EXIST(504,"修改仓库物流时发现该仓库或者仓库物流并不存在"),//3
    FREIGHT_SHOPLOGISTIC_EXIST(999,"商铺已存在物流(id=%d)"),//4


    FREIGHT_REGION_NOEXIST(709, "不存在配送地区(id=%d)"),//自己加的
    FREIGHT_REGION_EXIST(708, "仓库已存在该配送地区(id=%d)"),
    FREIGHT_WAREHOUSE_NOEXIST(800, "不存在仓库(id=%d)");//自己加的
