...
public class ShopController {
    ...
    */
    ...
    public ReturnObject updateShop(@PathVariable("id") Long id,
        ...
        ReturnObject ret = shopService.updateShop(id, vo.getConsignee().getName(), vo.getConsignee().getMobile(), vo.getConsignee().getRegionId(), vo.getConsignee().getAddress(), vo.getFreeThreshold(), user);
        return new ReturnObject(ret);
    }
    ...
    public ReturnObject deleteShop(@PathVariable("id") Long id, @LoginUser UserDto userDto){
        ReturnObject ret = shopService.deleteShop(id, userDto);
        return new ReturnObject(ret);
    }
    ...
    */
    @GetMapping("/{id}")
    @Audit(departName = "shops")
    public ReturnObject findShopById(@PathVariable("id") Long id, @LoginUser UserDto userDto){
        logger.debug("findShopById: id = {}", id);
        ShopDto shop = shopService.findShopById(id, userDto);
    ...
    */
    @PutMapping ("/{id}/audit")
    @Audit(departName = "shops")
    public ReturnObject updateShopAudit(@PathVariable("id") Long id, @LoginUser UserDto userDto, @Validated @RequestBody ShopAuditVo shopAuditVo){
        if(PLATFORM != userDto.getDepartId()) {
            return new ReturnObject(ReturnNo.RESOURCE_ID_OUTSCOPE, String.format(ReturnNo.RESOURCE_ID_OUTSCOPE.getMessage(), "商铺", id, userDto.getDepartId()));
        }
        ReturnObject ret = new ReturnObject();
        if(shopAuditVo.getConclusion())
             ret = shopService.updateShopStatus(id, Shop.OFFLINE, userDto);
        return new ReturnObject(ret);
    }
    ...
    */
    ...
    public ReturnObject updateShopOnline(@PathVariable("id") Long id, @LoginUser UserDto userDto){
        ...
        ReturnObject ret = shopService.updateShopStatus(id, Shop.ONLINE, userDto);
        return new ReturnObject(ret);
    }
    ...
    */
    ...
    public ReturnObject updateShopOffline(@PathVariable("id") Long id, @LoginUser UserDto userDto){
        ...
        ReturnObject ret = shopService.updateShopStatus(id, Shop.OFFLINE, userDto);
        return new ReturnObject(ret);
    }
    ...
    */
    ...
    public ReturnObject createProductService(@PathVariable("did") Long did, @PathVariable("id") Long pid,
        ...
        SimpleProductServiceDto ret = shopService.createProductService(did, pid, mid, rid, vo.getBeginTime(), vo.getEndTime(), vo.getInvalid(), vo.getPriority(), userDto);
        return new ReturnObject(ReturnNo.CREATED, ret);
    }
    ...
    */
    ...
    public ReturnObject retrieveProductServiceByProductIdAndRegionId(@PathVariable("did") Long did, @PathVariable("pid") Long pid,
                                                                     ...
                                                                     @RequestParam(required = false,defaultValue = "0") Integer page,
                                                                     @RequestParam(required = false,defaultValue = "10") Integer pageSize){
        PageDto<SimpleShopDto> shopDtoPageDto = shopService.retrieveProductServiceByProductIdAndRegionId(did, pid, rid, userDto, page, pageSize);
