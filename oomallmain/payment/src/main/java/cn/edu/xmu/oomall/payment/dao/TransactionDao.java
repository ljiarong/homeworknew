...
public class TransactionDao {
    ...
    public void saveById(Long transId, String className, Byte status) throws RuntimeException {
        if (className.equals(PayTrans.class.getName()))
            this.payTransDao.saveById(transId, status);
        else if (className.equals(RefundTrans.class.getName()))
            this.refundTransDao.saveById(transId, status);
    }
}
